...
- **普通聊天就是入口。** 监听服务器聊天；明确问候和动作请求先走快捷路由，其余由小模型判断 `ignore / reply / act`，只有 `act` 才唤醒高推理游戏 Agent。
...
- **真实执行游戏动作。** 假玩家身体与基础生存动作继续复用 Numen；移动/跟随、语义区域规划、任务验证与撤销逐步由 Momo Embodied 内核接管。
- **统一身体控制权。** 任务、原生寻路、战斗与生存本能共用独占租约；高优先级避险可以抢占，失去租约的控制器本 Tick 不再改写移动、视角或手部输入，任务恢复时也不会继承旧按键。
- **有硬上限的性能路径。** 空闲时不再逐 Tick 重建 17×17 区块快照；短时重规划复用快照，区块与方块实体元数据有固定上限，寻路线程和队列固定有界，原生碰撞采样与测试场地修改按时间和单位预算跨 Tick 续做。
- **服务端具身战斗监督。** 桃桃会结合实时生命/吸收、饥饿、已穿护甲、可用武器与盾、背包补给、Buff/Debuff、怪物数量与动作、视线、掩体和已认证安全屋，决定打、射、举盾、撤退或关门回屋；未知高风险怪不会默认冲上去近战。
- **可学习但不可越权的战术。** 服务端会保留有界战斗轨迹，Luna 可把重复出现的招式总结为精确绑定生物类型、观察适配器与版本的声明式策略；候选策略每次最多执行 160 tick，仍受服务端紧急避险硬约束，连续三次成功后才晋级，失败会停用。
- **可续跑、可局部修正的结构工作流。** 大型建造会保存精确蓝图和版本号；每个检查点后对照真实世界，放置失败时先预检支撑、朝向、占位与可站位置，再用带 revision 校验的局部 patch 修正，而不是重发整栋建筑或盲目重试。
- **按任务阶段收窄工具。** 聊天、勘测、采集、合成、建造、区域编辑、生存与战斗学习分别使用固定工具包；每个模型回合暴露的 Minecraft 与 Harness 工具合计不超过 18 个，可复用工作流的阶段只额外开放经过验证的 `run_skill`，减少误选工具与无效上下文。
- **目标级紧凑上下文。** 每个目标维护有界、结构化的 context capsule，保留关键状态、物品计数、空间范围与场景对象卡片，并在同一目标内发送增量；开始独立新目标时会重置，避免旧建筑或旧目标信息污染后续决策。
- **可恢复的 Harness 控制面。** 大脑状态、事件 inbox、回合开始/完成、动作回执与已处理输入会写入原子化持久快照；启动后先读取 Minecraft 的 `server_session_id` 并重新核对仍在运行的任务。会话改变、回执缺失或状态无法证明时一律 fail closed，只进入只读核对，不自动重放未知动作。
- **经过验证的 `run_skill`。** Skill runner 会固定 Skill 版本，校验参数类型、工具白名单、前置/逐步/异步终态/后置 verifier，并精确关联任务回执；候选 Skill 逐步验证，连续三次成功且没有失败才晋级为 trusted，晋级后仍保留最终结果验证、取消与预算约束。
- **可审计的预算与追踪。** 每个目标都有模型回合、工具调用、恢复回合与 Harness 活跃执行时间上限；等待异步 Minecraft 任务的时间不会误计为活跃执行时间。脱敏 NDJSON trace 只记录关联标识、阶段、结果与预算字段，并按大小轮转。
...
以上第二批 Harness 加固描述的是当前分支实现；生产服务器需同时部署匹配版本的 Numen API、Momo Embodied 与 `server-brain` 后生效。
...
- 客户端原有的“模型 / MCP / Skill”设置仍只管理**客户端内置大脑**；服务器 `server-brain` 的执行模型与思考强度目前由 OP/控制台通过 `/momo brain ...` 管理，客户端服务器管理模式仍是后续工作。
- 不建议在服务器 Agent 工作时从 `G` 面板聊天页启动客户端内置大脑；在控制权握手完成前，两套大脑可能同时向同一个身体下发任务。普通服务器聊天不受影响。
- `run_skill` 的验证执行、去重记录与三次成功晋级已经实现，但普通玩家回合**不开放** `save_skill_candidate`。把一次成功自动归纳成新 Skill 的模型写入闭环尚未开放；候选 Skill 目前应通过受控的内部或人工流程创建，一次成功也不会直接成为 trusted。
- Java 侧事件轮询目前仍会从内存队列中破坏性取出事件，因此还不是严格的端到端 exactly-once/claim-ack 协议。当前保障是 sidecar 的持久 journal/inbox、去重、动作回执、`server_session_id` 失效保护与只读恢复；若 Minecraft 进程在事件取出后、sidecar 持久化前崩溃，仍存在补偿恢复无法完全消除的窗口。严格保证需要后续把 durable claim/ack 下沉到 Java 侧。
- 当前重点是小型私人 Forge 1.20.1 服务器；长时间稳定性、更多模组和更多客户端环境仍需继续实测。
- `server-brain` 的 `MOMO_ACTIVITY_MODE` 默认是 `supervised`；只有显式改为 `autonomous` 才允许任务/身体遥测在没有玩家目标时自行开启后续回合。
...
以下命令要求权限等级 4，可由服务器控制台或 OP 执行，并且不依赖
Momo 身体已经召唤：
...
`status` 与 `list` 每次都会向 `server-brain` 请求新状态，命令最先展示的
内容仅是上次确认缓存。`set` 把模型与思考强度作为一个原子组合提交；控制台
显示“已提交”不代表已经切换，只有 sidecar 返回匹配 request id、未过期且
完整 active 组合一致的 applied 回执后，游戏内才会显示“已确认并生效”。
sidecar 不会应用排队超过 30 秒的请求；Forge 另留 30 秒给已经落盘但正在
重试的确认回执。最终仍无回执、模型不受支持、思考强度不兼容或 sidecar
拒绝应用时，原控制台/OP 会收到明确错误。

不进入游戏也可以从服务器面板控制台直接发消息：

```text
momo chat <内容>
```

这条子命令只接受服务器控制台/RCON，不接受游戏内玩家执行。消息仍走与普通
聊天相同的路由、感知和生存动作链；桃桃的回复会同时显示给在线玩家并镜像到
服务器控制台。控制台没有游戏内身体、位置或视线，因此“我这里”“这个方块”
之类依赖玩家视角的说法会要求补充目标，而不猜测一个虚构坐标。
