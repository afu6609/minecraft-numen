    ...
    public String description() {
        return "RESOURCE GATHERING ONLY: gather naturally occurring blocks by type and count. Never use "
                ...
                + "are gained or none remain nearby. No coordinates or goto needed. count is items, not "
                 ...
                 + "needed tier if nothing qualifies. It records its entry before excavation and does not "
                 + "finish until it has either returned there by a verified land route or reported an "
                 + "explicit TRAPPED result with both entry and final coordinates, unless explicitly "
                 + "cancelled: cancellation is immediate and does not guarantee a return. For an exact visible "
                 + "cell use break_block; for a "
